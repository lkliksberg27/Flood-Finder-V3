...
const getDepthLabel = (cm) => {
  if (cm <= 15) return 'Low';
  if (cm <= 40) return 'Moderate';
  if (cm <= 65) return 'High';
  return 'Severe';
};
...
export default function SensorPopup({ sensor, onClose }) {
  ...
  const blueColor = getWaterBlue(sensor.waterLevelCm);
  ...
  return (
    <div className="bg-[#151a2e] rounded-2xl p-4 min-w-[220px] border border-white/10 shadow-xl">
      ...
      <span
        ...
        style={{ backgroundColor: `${blueColor}26`, color: blueColor }}
      >
        {getDepthLabel(sensor.waterLevelCm)}
      </span>
