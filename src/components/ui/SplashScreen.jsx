...
export default function SplashScreen() {
  ...
  // Always keep the element in the DOM — just fade it out and disable interaction.
  // Removing a fixed z-[9999] element from the DOM on mobile can trigger a
  // browser repaint that causes WebGL context loss (kills the Mapbox canvas).
  return (
    <motion.div
      initial={{ opacity: 1 }}
      ...
      <div className="relative flex items-center justify-center">
        {[0, 1, 2].map(i => (
          <motion.div
            ...
            animate={visible ? { scale: 2.5, opacity: 0 } : { opacity: 0 }}
            transition={{
              duration: 2,
              delay: i * 0.5,
