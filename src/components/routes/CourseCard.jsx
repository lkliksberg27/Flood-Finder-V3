            ...
            {isEditing ? (
              <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                <input
                  ...
                  className="bg-[#0c1021] text-white px-3 py-2 rounded-lg text-base flex-1 border border-white/10 focus:border-blue-500 focus:outline-none"
                  autoFocus
                />
                <button onClick={saveName} className="p-2 text-emerald-400 hover:bg-emerald-500/20 rounded-lg min-w-[44px] min-h-[44px] flex items-center justify-center"><Check className="w-5 h-5" /></button>
                <button onClick={() => setIsEditing(false)} className="p-2 text-gray-400 hover:bg-gray-500/20 rounded-lg min-w-[44px] min-h-[44px] flex items-center justify-center"><X className="w-5 h-5" /></button>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                <h3 className="text-white font-semibold text-sm truncate">{course.name}</h3>
                <button onClick={e => { e.stopPropagation(); setIsEditing(true); }} className="p-2 text-gray-500 hover:text-gray-300 hover:bg-white/5 rounded-lg flex-shrink-0 min-w-[44px] min-h-[44px] flex items-center justify-center">
                ...
                <>
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      onClick={(e) => { e.stopPropagation(); handleGenerateRoutes(); }}
                      disabled={generatingRoutes}
                      className="flex-1 px-3 py-2.5 text-xs text-gray-400 hover:text-gray-300 hover:bg-white/5 rounded-lg min-h-[44px] transition-colors"
                    >
                ...
                {showDeleteConfirm ? (
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex gap-1.5">
                      <button onClick={(e) => { e.stopPropagation(); setShowDeleteConfirm(false); }} className="px-3 py-2 text-xs text-gray-400 hover:bg-white/5 rounded-lg min-h-[44px]">Cancel</button>
                      <button onClick={(e) => { e.stopPropagation(); onDeleteCourse(course.id); setShowDeleteConfirm(false); }} className="px-3 py-2 text-xs bg-red-500/20 text-red-400 hover:bg-red-500/30 rounded-lg min-h-[44px]">Delete</button>
                    </div>
                  </div>
                ) : (
                  <button onClick={(e) => { e.stopPropagation(); setShowDeleteConfirm(true); }} className="flex items-center gap-1.5 text-xs text-red-400 hover:bg-red-500/10 px-3 py-2 rounded-lg min-h-[44px] transition-colors">
